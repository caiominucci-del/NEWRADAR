...
@router.get("/interest")
async def interest(
    keyword: str = Query(..., min_length=1, description="Keyword para Google Trends (ex: Bolsonaro)"),
    window: Window = Query("today 3-m", description="Janela (now 7-d, today 1-m, today 3-m)"),
    cache: CacheStore = Depends(get_cache),
    settings: Settings = Depends(get_settings),
):
    """Série temporal de interesse (timeseries) por keyword."""
    result = await get_interest_over_time(keyword, window, cache, settings)
...
async def related(
    ...
    """Keywords relacionadas (rising/top) para a keyword."""
    result = await get_related_queries(keyword, cache, settings)
    return result.to_dict()
...
async def macro(
    ...
    """Top 10 buscas do Brasil agora (trending macro)."""
    result = await get_macro_trends(cache, settings)
    return result.to_dict()